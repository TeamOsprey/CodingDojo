...
- Questions:
   ...
   - Shall we ask the developer agent to read the project plan or shall we ask architect agent to add the instruction into the schema file?      - If the developer is not supposed to have access to all the project plan, shall we store the implementation phases/user stories into a separate file and let developer to access those files but no the whole project plan?
...
We then asked the agent to explain this behavior. According to its response, the instruction that GitHub Copilot created from the __Rules__ section of _ochestration.agent.md_ file overrode the __"send: false"__ setting in the handoffs. The conflicting instruction was a rule requiring tests to be run after any code change. As a result, the agent interpreted this as a requirement to execute the preceding phases so that it could generate code and run the associated tests.  
While this explanation did not fully answer our question, we agreed that further testing could have been useful. For example, we could have rephrased the rules to make it explicit that tests should not be executed until the final phase, after all development work has been completed. At this point we decided to continue with the exercise and do not do further investigation.
- Next session we can continue working on __Exercise # 7__ from step __5. Try the category feature__.
